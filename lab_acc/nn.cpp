...
void Argmax(float *A, int *D, int n, int m) {
    #pragma acc data copyin(A[0:n*m]) copyout(D[0:n])
    {
        ...
        for (int i = 0; i < n; i++) {
            ...
            #pragma acc loop reduction(max:maxVal) reduction(max:maxIndex)
            for (int j = 1; j < m; j++) {
                if (A[i * m + j] > maxVal) {
                    maxVal = A[i * m + j];
                    maxIndex = j;
...
#pragma acc enter data create(layer1_output[0:num_images*LAYER1], layer2_output[0:num_images*LAYER2])
    #pragma acc enter data copyin(training_images_flat[0:num_images*LAYER0], layer1_matrix[0:LAYER0*LAYER1], layer1_bias[0:LAYER1], layer2_matrix[0:LAYER1*LAYER2], layer2_bias[0:LAYER2])
    ...
    #pragma acc exit data delete(layer1_output[0:num_images*LAYER1], layer2_output[0:num_images*LAYER2])
